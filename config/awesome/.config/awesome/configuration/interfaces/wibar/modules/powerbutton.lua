...
awful.spawn.easy_async_with_shell('whoami', function (whoami)
  username:set_markup_silently('Hey ' .. tostring(whoami) )
end)
...
local function make_powerbutton (opts)
  ...
  opts.bg = opts.bg and opts.bg or beautiful.bg_contrast

  local call_widget = function ()
    local icon_font = beautiful.font .. ' 32'
    local align = 'center'
  ...
  if opts.bg == beautiful.bg_contrast then
    helpers.add_hover(button, beautiful.bg_contrast, beautiful.lightblack)
  end
...
return helpers.mkbutton(wibox.widget {
  powerbutton,
  widget = wibox.container.background,
}, beautiful.powerbutton_bg, beautiful.powerbutton_bg)
