...
if [[ "$answer" =~ ^[Yy]$ ]]; then
	...
	[[ -z $(command -v zsh) ]]     && echo -e "INSTALLING ${YELLOW}ZSH${NC}" && sudo apt install -y zsh && echo -e "${GREEN}DONE${NC}";
	...
	[[ -z $(command -v btop) ]]    && echo -e "INSTALLING ${YELLOW}BTOP${NC}" && sudo apt install -y btop && echo -e "${GREEN}DONE${NC}";
	[[ -z $(command -v batcat) ]]  && echo -e "INSTALLING ${YELLOW}BAT${NC}" && sudo apt install -y bat && echo -e "${GREEN}DONE${NC}";
else
	echo "Aborting."
	exit 1
fi
